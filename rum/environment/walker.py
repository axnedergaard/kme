...
def run_sparse(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  """Returns the Run task."""
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = PlanarWalker(move_speed=_RUN_SPEED, random=random, sparse=True)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
class PlanarWalker(base.Task):
  ...
  def __init__(self, move_speed, random=None, sparse=False):
  ...
  def get_reward(self, physics):
    ...
    if not self.sparse or reward > 0.25:
      return reward
    else:
      return 0.0