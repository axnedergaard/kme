...
def run_sparse(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  ...
  xml_string = make_model(floor_size=_DEFAULT_TIME_LIMIT * _RUN_SPEED)
  physics = Physics.from_xml_string(xml_string, common.ASSETS)
  task = Move(desired_speed=_RUN_SPEED, random=random, sparse=True)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(physics, task, time_limit=time_limit,
                             control_timestep=_CONTROL_TIMESTEP,
                             **environment_kwargs)
...
class Move(base.Task):
  ...
  def __init__(self, desired_speed, random=None, sparse=False, reward_threshold=0.7):
  ...
  def get_reward(self, physics):
    ...
    reward = _upright_reward(physics) * move_reward
    if not self.sparse or reward > self.reward_threshold:
      return reward
    else:
      return 0.0
