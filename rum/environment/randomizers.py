...
def randomize_limited_and_rotational_joints(physics, random=None, maze=False):
      ...
      if joint_type == hinge or joint_type == slide:
        if maze:
          if joint_name == 'root_x':
            qpos[joint_name] = random.uniform(range_min, -0.14)
          elif joint_name == 'root_y':
            qpos[joint_name] = random.uniform(0.14, 0.29)
        else:
